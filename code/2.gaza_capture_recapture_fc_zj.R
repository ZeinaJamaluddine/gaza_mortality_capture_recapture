...
#...............................................................................
  ...
  ## Save imputed datasets for each age-sex stratum (for RShiny app model
    ...
    for (i in 1:nrow(median_table)) {
      ...
      aggregated_results <- aggregate(cbind(unlisted_est, unlisted_lci, unlisted_uci) ~ n_imp, data = model_results, FUN = sum)
      
      # Calculate medians of deaths and confidence intervals across all imputations
      median_table$Median_Deaths[i] <- median(aggregated_results$unlisted_est, na.rm = TRUE)
      median_table$CI_Lower[i] <- median(aggregated_results$unlisted_lci, na.rm = TRUE)
      median_table$CI_Upper[i] <- median(aggregated_results$unlisted_uci, na.rm = TRUE)
      
      # Sum AIC by imputation and then take the median of those sums across all imputations
      aic_by_imp <- aggregate(aic ~ n_imp, data = model_results, FUN = sum)
      median_table$Median_AIC[i] <- median(aic_by_imp$aic, na.rm = TRUE)
      
      # Calculate mean posterior probability by imputation and then take the median across all imputations
      post_prob_by_imp <- aggregate(post_prob ~ n_imp, data = model_results, FUN = mean)
      median_table$Median_Posterior_Prob[i] <- median(post_prob_by_imp$post_prob, na.rm = TRUE)
    }
    
    # Format the output table for display
    median_table$Deaths_CI <- paste0(formatC(median_table$Median_Deaths, format="f", big.mark=",", digits=1),
                                     " (", formatC(median_table$CI_Lower, format="f", big.mark=",", digits=1),
                                     " to ", formatC(median_table$CI_Upper, format="f", big.mark=",", digits=1), ")")
    
    median_table$AIC <- formatC(median_table$Median_AIC, format="f", big.mark=",", digits=1)
    median_table$Posterior_Prob <- formatC(median_table$Median_Posterior_Prob, format="f", digits=3)
    ...
    write.csv(final_output, paste0(dir_path, "output/median_model_results_unadj.csv"), row.names = FALSE)
#...............................................................................
### ENDS
#...............................................................................
