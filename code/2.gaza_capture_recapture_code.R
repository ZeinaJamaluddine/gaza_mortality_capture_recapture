  ...
  ## Compare distributions of imputed and non-missing values
    ...
    pl_month <- ggplot(combi_month_death,
      ...
      scale_x_discrete("month of death", 
        limits = c("Oct","Nov","Dec","Jan","Feb","Mar","Apr","May","Jun")) +
      scale_y_continuous("frequency", labels = comma) +
      theme_bw() +
...
#...............................................................................
  ...
  #...................................      
  ## Visualise average models
    
    # Create a placeholder for storing medians and sums
    median_table <- data.frame(
      Model = c("no interactions", 
        "hospitals x social media, hospitals x survey, social media x survey",
        "hospitals x social media, hospitals x survey",
        "hospitals x social media, social media x survey",
        "hospitals x survey, social media x survey",
        "hospitals x social media", 
        "hospitals x survey", 
        "social media x survey"),
      Median_Deaths = NA,
      CI_Lower = NA,
      CI_Upper = NA,
      Median_AIC = NA,
      Median_Posterior_Prob = NA
    )
    
    
    # Loop through each model to calculate medians and sums across imputations if eligibility is TRUE
    for (i in 1:nrow(median_table)) {
      model_name <- median_table$Model[i]
      
      # Filter results for the specific model across all imputations, checking eligibility
      model_results <- est_agesex_unadj[est_agesex_unadj$model == model_name & 
                                          est_agesex_unadj$eligible == TRUE, ]
      
      # Aggregate unlisted estimates (deaths) and their CIs by imputation number
      aggregated_results <- aggregate(cbind(unlisted_est, unlisted_lci, 
                                            unlisted_uci) ~ n_imp, data = model_results, FUN = sum)
      
      # Calculate medians of deaths and CIs across all imputations
      median_table$Median_Deaths[i] <- 
        median(aggregated_results$unlisted_est, na.rm = T)
      median_table$CI_Lower[i] <- 
        median(aggregated_results$unlisted_lci, na.rm = T)
      median_table$CI_Upper[i] <- 
        median(aggregated_results$unlisted_uci, na.rm = T)
      
      # Sum AIC by imputation, take median of those sums across all imputations
      aic_by_imp <- aggregate(aic ~ n_imp, data = model_results, FUN = sum)
      median_table$Median_AIC[i] <- median(aic_by_imp$aic, na.rm = T)
      
      # Calculate mean posterior probability by imputation, then take median
      post_prob_by_imp <- aggregate(post_prob ~ n_imp, 
                                    data = model_results, FUN = mean)
      median_table$Median_Posterior_Prob[i] <- 
        median(post_prob_by_imp$post_prob, na.rm = T)
    }
    
    # Format the output table for display
    median_table$Deaths_CI <- paste0(
      formatC(median_table$Median_Deaths, format="f", big.mark=",", digits=1),
      " (", 
      formatC(median_table$CI_Lower, format="f", big.mark=",", digits=1),
      " to ", 
      formatC(median_table$CI_Upper, format="f", big.mark=",", digits=1),
      ")"
    )
    
    median_table$AIC <- formatC(median_table$Median_AIC, format="f", 
                                big.mark=",", digits=1)
    median_table$Posterior_Prob <- formatC(median_table$Median_Posterior_Prob, 
                                           format="f", digits=3)
    
    # Select only relevant columns for final output
    final_output <- median_table[, c("Model","Deaths_CI","AIC","Posterior_Prob")]
    
    # Print the resulting table of medians
    print(final_output)
    
    # Save the table to a CSV file
    write.csv(final_output, paste0(dir_path, 
                                   "output/median_model_results_unadj.csv"), row.names = F)
    

#...............................................................................
### Testing alternative model approaches
#...............................................................................

  #...................................      
  ## Mixed-logit model (= quasi-symmetry log-linear model)
  
    # Read and format all aggregate stratum contingency tables
    x <- list.files(paste0(dir_path, "for_rshiny"))
    strata <- vector("list", length = length(x))
    names_list <- gsub(" ", "_", x)
    names_list <- gsub(".csv", "", names_list)
    names(strata) <- names_list
    for (i in 1:length(x)) {
      df <- read.csv(paste0(dir_path, "for_rshiny/", x[i]))
      df$outcome <- factor(paste0("x", paste0(df[, 1], df[, 2], df[, 3])),
        levels = c("x111", "x110", "x101", "x100", "x011", "x010", "x001"))
      df$count <- df[, 4]
      df <- df[order(df$outcome), ]
      strata[[i]] <- df[, c("outcome", "count")]
    }
    
    # Initialise output
    out <- data.frame(stratum = gsub(".csv", "", x), 
      stratum_name = names(strata), listed = NA,
      unlisted_est = NA, unlisted_lci = NA, 
      unlisted_uci = NA)
    for (i in names(strata)) {out[which(out$stratum_name == i), "listed"] <-
      sum(strata[[i]]["count"])}
    
    # Dummy data for prediction
    df0 <- data.frame(x12 = 0, x13 = 0, x23 = 0, x1 = 0, x2 = 0, x3 = 0)
    
    # Fit quasi-symmetry model and predict unlisted
    for (i in names(strata)) {
      df <- strata[[i]]
      df$x1 <- ifelse(substr(df$outcome, 2, 2) == 1, 1, 0)
      df$x2 <- ifelse(substr(df$outcome, 3, 3) == 1, 1, 0)
      df$x3 <- ifelse(substr(df$outcome, 4, 4) == 1, 1, 0)
      df$x12 <- ifelse(df$outcome %in% c("x111", "x110"), 1, 0)
      df$x13 <- ifelse(df$outcome %in% c("x111", "x101"), 1, 0)
      df$x23 <- ifelse(df$outcome %in% c("x111", "x011"), 1, 0)
      fit <- glm(count ~ x1 + x2 + x3 + x12 + x13 + x23, 
        family = poisson(link = 'log'), data = df)
      out[which(out$stratum_name == i), "unlisted_est"] <- 
        round(predict(fit, newdata = df0, type = "response"), 0)
      x <- confint(fit)
      out[which(out$stratum_name == i), "unlisted_lci"] <- round(exp(x[1, 1]),0)
      out[which(out$stratum_name == i), "unlisted_uci"] <- round(exp(x[1, 2]),0)
    }
    
    # Compute totals and save
    out$total_est <- out$listed + out$unlisted_est
    out$total_lci <- out$listed + out$unlisted_lci
    out$total_uci <- out$listed + out$unlisted_uci
    write.csv(out, paste0(dir_path, "output/mixed_logit_model.csv"),row.names=F)
    
  #...................................      
  ## Bayesian Model Average (dga package)
    #install 
